...
import { escapeChars, replaceFilename } from '../../../libs/utils/index.js';
...
export function packagesIndex(model) {
    ...
    if (this.options.getValue('indexFormat').toLowerCase().includes('table')) {
        ...
        const packageRows = model.children?.map((projectPackage) => {
            ...
            const rows = [
                link(`${escapeChars(projectPackage.name)}`, getPackageLink(this, projectPackage)),
            ];
            if (includeVersion) {
        ...
        const packagesList = model.children?.map((projectPackage) => {
            return this.router.hasUrl(projectPackage)
                ? `- ${link(`${escapeChars(projectPackage.name)}${projectPackage.packageVersion ? ` - v${projectPackage.packageVersion}` : ''}`, getPackageLink(this, projectPackage))}`
                : '';
        });
...
function getPackageLink(context, projectPackage) {
    return projectPackage.readme
        ? replaceFilename(context.urlTo(projectPackage), context.router.getPackageEntryFileName(projectPackage) || '')
